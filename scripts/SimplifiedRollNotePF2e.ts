import { RuleElement, DegreeOfSuccessString, RawPredicate } from 'pf2e-types'
import { createHTMLElement } from './htmlHelpers.ts';

const USER_VISIBILITIES = new Set(["all", "owner", "gm", "none"] as const);
type UserVisibility = SetElement<typeof USER_VISIBILITIES>;

class SimplifiedRollNotePF2e {
    ...
    //predicate: Predicate; // Nie potrzebujemy, więc zakomentowane
    ...
    constructor(params: SimplifiedRollNoteParams) {
    ...
    static notesToHTML(notes: SimplifiedRollNotePF2e[]): HTMLUListElement | null {
...
interface SimplifiedRollNoteSource {
    selector: string;
    title?: string | null;
    text: string;
...
interface SimplifiedRollNoteParams extends SimplifiedRollNoteSource {
    rule?: RuleElement | null;
}

export { SimplifiedRollNotePF2e, type SimplifiedRollNoteSource };